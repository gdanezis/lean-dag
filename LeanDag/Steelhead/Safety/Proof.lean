...
commit, and SH4 is definitional up to the two kind lemmas and
Mahi-Mahi's wave-three correspondence, whose converse
`decided_of_core_decided` mirrors it.
-/
...
theorem holds : Statement := by
  ...
  · intro V₁ V₂ k L hw hL hc
    ...
    · exact AnchoredRule.Decided.indirectCommit_single rfl (fun _ _ h => h) hkj helig hj hmid hL
        (certifiedIn_of_commit_at_anchor (fun κ => by have := hw κ; omega) hc
