...
/-- **Agreement of the period under two handovers** that agree at every anchor one of the views
finds below the interval: induction on the derivation; the anchor is common, the outputs handed
at it are equal, so is the update. -/
theorem periodAt_unique_of_out (hwa : 3 ≤ wa) {upd : UpdateRule BlockId} {k₀ : ℕ}
    {V₁ V₂ : View Validator BlockId Payload U} {out₁ out₂ : BlockId → Finset BlockId}
    {j k₁ k₂ : ℕ} (h₁ : PeriodAt I wa coin upd k₀ U V₁ out₁ j k₁)
    (h₂ : PeriodAt I wa coin upd k₀ U V₂ out₂ j k₂)
    (hout : ∀ j' k r A, j' < j → IntervalAnchor I wa coin U V₁ j' k r A → out₁ A = out₂ A) :
    k₁ = k₂ := by
  induction h₁ generalizing k₂ with
  | zero => cases h₂; rfl
  | @anchor j k r A hp ha ih =>
    cases h₂ with
    | anchor hp' ha' =>
      obtain rfl := ih hp' fun j' k r A hj => hout j' k r A (by omega)
      obtain ⟨rfl, rfl⟩ := ha.unique hwa ha'
      rw [hout j k r A (by omega) ha]
    | keep hp' hn =>
      obtain rfl := ih hp' fun j' k r A hj => hout j' k r A (by omega)
  ...
  | keep hp hn ih =>
    ...
    | anchor hp' ha' =>
      obtain rfl := ih hp' fun j' k r A hj => hout j' k r A (by omega)
      exact (ha'.not_noAnchor hwa hn).elim
    | keep hp' _ => exact ih hp' fun j' k r A hj => hout j' k r A (by omega)

/-- **SH10a.** Both views hand the rule the same outputs. -/
theorem periodAt_unique (hwa : 3 ≤ wa) {upd : UpdateRule BlockId} {k₀ : ℕ}
    ...
    (h₁ : PeriodAt I wa coin upd k₀ U V₁ out j k₁) (h₂ : PeriodAt I wa coin upd k₀ U V₂ out j k₂) :
    k₁ = k₂ :=
  periodAt_unique_of_out hwa h₁ h₂ fun _ _ _ _ _ _ => rfl
...
The window's output is read in the anchor's causal history; the validator holds a view. The two
...
/-- **The window's output reads the wavelength at or below the anchor's round only.** Every
verdict the test reads is at a slot the anchor's history commits a block of, or below one, and
the history holds no block above the anchor's round; so two wavelength functions agreeing up to
that round give one output. -/
theorem windowOutput_congr {w₁ w₂ : ℕ → ℕ} {A : BlockId} (hA : A ∈ U.ids)
    (hw : ∀ r, r ≤ (U.block A).round → w₁ r = w₂ r) :
    windowOutput U w₁ I A hA = windowOutput U w₂ I A hA := by
  -- a verdict at a committed slot of the history, or at a slot below it, transfers
  have key : ∀ {w₁ w₂ : ℕ → ℕ}, (∀ r, r ≤ (U.block A).round → w₁ r = w₂ r) →
      ∀ {s : ℕ} {L : BlockId}, Decided w₁ U (U.historyView A hA) s (some L) →
      Decided w₂ U (U.historyView A hA) s (some L) ∧
        ∀ s' < s, ∀ {v : Option BlockId}, Decided w₁ U (U.historyView A hA) s' v →
          Decided w₂ U (U.historyView A hA) s' v := by
    intro w₁ w₂ hw s L h
    have hN : ∀ b ∈ (U.historyView A hA).ids, (U.block b).round ≤ (U.block A).round :=
      fun b hb => round_le_of_mem_history hA hb
    have hs : S.slotRound s ≤ (U.block A).round := by
      rw [← (AnchoredRule.isLeaderBlock_of_decided h).2.1]
      exact hN L (mem_ids_of_decided h L rfl)
    exact ⟨decided_congr hN hw h hs,
      fun s' hs' v hv => decided_congr hN hw hv (le_trans (S.mono hs'.le) hs)⟩
  classical
  unfold windowOutput
  refine Finset.filter_congr fun L _ => ⟨?_, ?_⟩
  · rintro ⟨s, hb, hd, hlow⟩
    obtain ⟨hd', hlow'⟩ := key hw hd
    exact ⟨s, hb, hd', fun s' hs' => (hlow s' hs').imp fun _ hv => hlow' s' hs' hv⟩
  · rintro ⟨s, hb, hd, hlow⟩
    obtain ⟨hd', hlow'⟩ := key (fun r hr => (hw r hr).symm) hd
    exact ⟨s, hb, hd', fun s' hs' => (hlow s' hs').imp fun _ hv => hlow' s' hs' hv⟩

/-- **SH10b.** The sequences coincide below the record's top interval, by strong induction: the
anchors of the intervals below an interval lie in the record, their windows are read at rounds
below their own, where the sequences already agree, so the two views hand the rule the same
output at every one of them and derive the same period. The two views then run one wavelength
...
theorem adaptive_decided_unique {ws : ℕ} (hws : 3 ≤ ws) (hwa : 3 ≤ wa) {upd : UpdateRule BlockId}
  ...
  have hper : ∀ j, j ≤ intervalOf I N → per₁ j = per₂ j := by
    ...
    | _ j ih =>
    intro hj
    refine periodAt_unique_of_out hwa (h₁ j hj) (h₂ j hj) fun j' k r A hj' hA => ?_
    have hAU : A ∈ U.ids :=
    ...
    simp only [adaptiveOutput, dif_pos hAU]
    refine windowOutput_congr hAU fun r hr =>
      adaptiveWave_congr (ws := ws) (N := (U.block A).round) (fun j'' hj'' => ?_) hr
    rw [hAr, hA.mem] at hj''
...
theorem exists_periodAt_succ {upd : UpdateRule BlockId} {k₀ : ℕ}
    ...
    (hp : PeriodAt I wa coin upd k₀ U V out j k)
    (hall : ∀ r, intervalOf I r = j → IsAsync k r → ∃ v, ChainDecided wa coin U V r v) :
    ∃ k', PeriodAt I wa coin upd k₀ U V out (j + 1) k' := by
  classical
...
/-- **The window of an anchor the view did not output is handed over empty.** What the view
...
So no leader of the window passes the output's test in the history. -/
theorem adaptiveOutput_eq_empty (hws : 2 ≤ ws) (hwa : 2 ≤ wa) {j k r : ℕ} {A : BlockId}
    ...
    adaptiveOutput ws wa I per U A = ∅ := by
  have hAU : A ∈ U.ids :=
    (AnchoredRule.isLeaderBlock_of_decided (S := chainSlots coin) hA.commit).1
  ...
  simp only [adaptiveOutput, dif_pos hAU]
  unfold windowOutput
  refine Finset.filter_eq_empty_iff.mpr fun L _ h => ?_
  obtain ⟨s, hs, hd, hlow⟩ := h
  obtain ⟨s', hlt, hund⟩ := hout s L hs (hmono hd)
  obtain ⟨v, hv⟩ := hlow s' hlt
  exact hund v (hmono hv)

/-- **The failover fires at an anchor whose window the view did not output**: the rule is handed
an empty output, and answers `1`. -/
theorem upd_eq_one_of_anchor (hws : 2 ≤ ws) (hwa : 2 ≤ wa) (hreset : ResetsOnNoOutput upd)
    {j k r : ℕ} {A : BlockId} (hA : IntervalAnchor I wa coin U V j k r A)
    (hout : ∀ (s : ℕ) (L : BlockId), windowBottom U A I ≤ S.slotRound s →
      Decided (adaptiveWave ws wa I per) U V s (some L) →
      ∃ s', s' < s ∧ ∀ v, ¬ Decided (adaptiveWave ws wa I per) U V s' v) :
    upd j A (adaptiveOutput ws wa I per U A) k = 1 := by
  rw [adaptiveOutput_eq_empty hws hwa hA hout]
  exact hreset j A k

/-- **SH10e.** The anchor step of the sequence, its update read off the failover. -/
theorem periodAt_one_of_anchor (hws : 2 ≤ ws) (hwa : 2 ≤ wa) (hreset : ResetsOnNoOutput upd)
    {j k r : ℕ} {A : BlockId}
    (hp : PeriodAt I wa coin upd k₀ U V (adaptiveOutput ws wa I per U) j k)
    (hA : IntervalAnchor I wa coin U V j k r A)
    (hout : ∀ (s : ℕ) (L : BlockId), windowBottom U A I ≤ S.slotRound s →
      Decided (adaptiveWave ws wa I per) U V s (some L) →
      ∃ s', s' < s ∧ ∀ v, ¬ Decided (adaptiveWave ws wa I per) U V s' v) :
...
theorem exists_periodAt_of_settled {upd : UpdateRule BlockId} {k₀ : ℕ}
    {V : View Validator BlockId Payload U} {out : BlockId → Finset BlockId} {n : ℕ}
    (hall : ∀ r, intervalOf I r ≤ n → ∃ v, ChainDecided wa coin U V r v) :
    ∀ j, j ≤ n + 1 → ∃ k, PeriodAt I wa coin upd k₀ U V out j k := by
  intro j
  induction j with
...
theorem periodAt_mem_range {K : ℕ} {upd : UpdateRule BlockId} {k₀ : ℕ}
    {V : View Validator BlockId Payload U} {out : BlockId → Finset BlockId} (h₀ : 1 ≤ k₀)
    (hK : k₀ ≤ K) (hupd : ∀ j A out k, 1 ≤ k → k ≤ K → 1 ≤ upd j A out k ∧ upd j A out k ≤ K)
    {j k : ℕ} (hp : PeriodAt I wa coin upd k₀ U V out j k) : 1 ≤ k ∧ k ≤ K := by
  induction hp with
  | zero => exact ⟨h₀, hK⟩
  | anchor _ _ ih => exact hupd _ _ _ _ ih.1 ih.2
  | keep _ _ ih => exact ih

/-- **SH10h.** The wrapper answers `1` exactly where the clause asks it to. -/
theorem failover_resets (upd : UpdateRule BlockId) : ResetsOnNoOutput (failover upd) :=
  fun _ _ _ => if_pos rfl
...
theorem PeriodAt.succ_cases {upd : UpdateRule BlockId} {k₀ : ℕ}
    {V : View Validator BlockId Payload U} {out : BlockId → Finset BlockId} {j k' : ℕ}
    (h : PeriodAt I wa coin upd k₀ U V out (j + 1) k') :
    (∃ k r A, PeriodAt I wa coin upd k₀ U V out j k ∧ IntervalAnchor I wa coin U V j k r A ∧
        k' = upd j A (out A) k) ∨
      ∃ k, PeriodAt I wa coin upd k₀ U V out j k ∧ NoAnchor I wa coin U V j k ∧ k' = k := by
  cases h with
...
theorem output_liveness (hws : 2 ≤ ws) (hle : ws ≤ wa) (hwa : 3 ≤ wa)
    (hid : ∀ t, S.slotRound t = t) (hI : 0 < I) (hreset : ResetsOnNoOutput upd) {b : ℕ}
    (hper : ∀ j, j ≤ intervalOf I (b + wa - 1) →
      PeriodAt I wa coin upd k₀ U V (adaptiveOutput ws wa I per U) j (per j))
...
theorem output_liveness_of_runs (hws : 2 ≤ ws) (hle : ws ≤ wa) (hwa : 3 ≤ wa)
    ...
    (hK : k₀ ≤ K) (hupd : ∀ j A out k, 1 ≤ k → k ≤ K → 1 ≤ upd j A out k ∧ upd j A out k ≤ K)
    (hKI : K ≤ I) (hreset : ResetsOnNoOutput upd) {b : ℕ}
    (hper : ∀ j', j' ≤ intervalOf I (b + wa - 1) →
      PeriodAt I wa coin upd k₀ U V (adaptiveOutput ws wa I per U) j' (per j'))
