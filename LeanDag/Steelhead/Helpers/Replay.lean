...
period and only ever moves to a candidate, whether or not the current period is one. -/
theorem select_eq_or_mem (candidates : List ℕ) (scores : ℕ → ℚ) (current : ℕ) (epsilon : ℚ) :
  ...
  have hbest : best candidates scores current = current ∨
      best candidates scores current ∈ candidates :=
    List.mem_cons.mp (fold_mem (current :: candidates) candidates scores current current
...
/-- **SH18h.** The selection answers the current period or a candidate, so it stays in the range
the candidates and the current period share. -/
theorem anchorUpdate_range {U : BlockUniverse Validator BlockId Payload} {I : ℕ}
    ...
    1 ≤ anchorUpdate U I C candidates epsilon A k ∧
      anchorUpdate U I C candidates epsilon A k ≤ K := by
...
of them. -/
theorem probe_exists {E : Evidence Validator} {C : Config Validator} {canary period : ℕ}
    (hcan : C.canary = some canary) (hcop : Nat.Coprime canary period) (hp : 2 ≤ period)
    (hws : 1 ≤ C.ws) (hwin : E.bottom + 2 * canary + C.ws ≤ E.top + 2) :
    0 < (probeRate E C period).2 := by
  have hc : 1 ≤ canary := by
    rcases Nat.eq_zero_or_pos canary with h | h
    · rw [h, Nat.coprime_zero_left] at hcop; omega
    · exact h
  -- the first multiple of the canary at or above the bottom
  set q := (E.bottom + canary - 1) / canary with hq
  have hdm := Nat.div_add_mod (E.bottom + canary - 1) canary
  have hml := Nat.mod_lt (E.bottom + canary - 1) (by omega : 0 < canary)
  rw [← hq] at hdm
  have hlo : E.bottom ≤ canary * q := by omega
  have hhi : canary * q < E.bottom + canary := by omega
  -- it or the next multiple is a probe
  have key : ∃ r, E.bottom ≤ r ∧ r + C.ws - 1 ≤ E.top ∧ r % canary = 0 ∧ r % period ≠ 0 := by
    by_cases h₁ : canary * q % period = 0
    · refine ⟨canary * q + canary, by omega, by omega, ?_, ?_⟩
      · rw [← Nat.mul_succ]; exact Nat.mul_mod_right _ _
      · intro h₂
        have hd : period ∣ canary :=
          (Nat.dvd_add_right (Nat.dvd_of_mod_eq_zero h₁)).mp (Nat.dvd_of_mod_eq_zero h₂)
        have := Nat.Coprime.eq_one_of_dvd hcop.symm hd
        omega
    · exact ⟨canary * q, hlo, by omega, Nat.mul_mod_right _ _, h₁⟩
  obtain ⟨r, hr₁, hr₂, hr₃, hr₄⟩ := key
  refine List.length_pos_of_mem (a := r) ?_
  rw [List.mem_filter, mem_rounds]
  refine ⟨⟨hr₁, by omega⟩, ?_⟩
  simp [Config.canaried, hcan, hr₂, hr₃, hr₄]
...
theorem bounded_roundTiming {E : Evidence Validator} {C : Config Validator} {period : ℕ}
  ...
  have hsync : ¬ (r % period = 0) → (if r % period = 0 then C.wa else C.ws) = C.ws := fun h =>
    if_neg h
  have hne : (if r % period = 0 then (Finset.univ : Finset Validator)
      else {C.known r}).Nonempty := by
    split_ifs
    · exact Finset.univ_nonempty
    · exact Finset.singleton_nonempty _
  generalize (if r % period = 0 then C.wa else C.ws) = wave at hwave hsync ⊢
  generalize (if r % period = 0 then (Finset.univ : Finset Validator) else {C.known r}) = authors
    at hne ⊢
  split_ifs with hclip hprobe
  · exact bounded_clipped hr
  · -- the unprobed synchronous slot: a mean of two rounds inside the window
    obtain ⟨hasync, -, ht⟩ := hprobe
    rw [hsync hasync] at hclip
...
theorem roundTiming_async_commit_le {E : Evidence Validator} {C : Config Validator} {period : ℕ}
  ...
  have hnp : ¬ (¬ (r % period = 0) ∧ ¬ C.canaried r ∧ probes.2 > 0) := fun h => h.1 hr
  unfold roundTiming
  dsimp only
  rw [if_pos hr, if_neg (by omega), if_neg hnp, if_pos hr]
  dsimp only
...
theorem select_eq_current_of_no_improvement (candidates : List ℕ) (scores : ℕ → ℚ) (current : ℕ)
  ...
  dsimp only [select]
  rw [if_neg (h _ (best_mem candidates scores current hc)), if_pos hc]
...
least half of what period `4` can score on a window of at most nine rounds, and hysteresis `1/2`
never moves. The scan's failover is what leaves such a period, not the replay (`PeriodAt.anchor`,
`Model/Period.lean`). -/
...
theorem halfFloor_le_roundTiming_commit {E : Evidence Validator} {C : Config Validator}
    ...
    (hcan : C.canary = some 1) (hp : period = 1 ∨ period = 2)
  ...
  · simp only [clipped]
    exact_mod_cast Nat.min_le_right _ _
  · exact absurd (show C.canaried r = true by simp [Config.canaried, hcan, Nat.mod_one])
      hprobe.2.1
...
theorem halfFloor_le_timingAt_commit {E : Evidence Validator} {C : Config Validator} {period : ℕ}
    (hws : C.ws = 3) (hwa : C.wa = 5) (hcan : C.canary = some 1) (hp : period = 1 ∨ period = 2) :
...
omit [Fintype Validator] [DecidableEq Validator] F [LinearOrder BlockId] in
/-- On a window of at most nine rounds, the half floor's excess over the round sums to at least
half the delays to the top, with equality at nine. -/
theorem half_floor_sum {E : Evidence Validator} (hw : E.bottom ≤ E.top)
    (hspan : E.top ≤ E.bottom + 8) :
...
/-- **Half hysteresis retains period four** on any window of at most nine rounds at waves `3`
...
theorem update_half_retains {E : Evidence Validator} {C : Config Validator} (hws : C.ws = 3)
    (hwa : C.wa = 5) (hcan : C.canary = some 1) (hw : E.bottom ≤ E.top)
    (hspan : E.top ≤ E.bottom + 8) : update E C [1, 2, 4] 4 (1 / 2) = 4 := by
...
startup windows included: the window of an anchor spans at most nine rounds, and an empty one
scores zero at every period. Only the scan's failover leaves such a period. -/
theorem anchorUpdate_half_retains (U : BlockUniverse Validator BlockId Payload)
    (C : Config Validator) (hws : C.ws = 3) (hwa : C.wa = 5) (hcan : C.canary = some 1)
    (A : BlockId) :
    anchorUpdate U 8 C [1, 2, 4] (1 / 2) A 4 = 4 := by
  ...
  · exact update_half_retains hws hwa hcan hw
      (by change (U.block A).round ≤ max 1 ((U.block A).round - 8) + 8; omega)
  · apply select_eq_current_of_no_improvement _ _ _ _ (by simp)
    intro k _
