...
The paper's Algorithm 2 as data, as `replay.rs` computes it: the
...
the factor `1 − ε`, the current period otherwise; the best candidate outright when the current
period is not a candidate, as `choose_period` has it, though the implementation never reaches
that case. -/
