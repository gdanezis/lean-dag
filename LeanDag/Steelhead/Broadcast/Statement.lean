...
ledger theorems (SH13) and the liveness theorems make them true. Five
claims:
...
/-- **SH17e, validity under asynchrony.** -/
def ValidityOfEventualReference (U : BlockUniverse Validator BlockId Payload) (w : ℕ → ℕ) :
    Prop :=
...
def Statement : Prop :=
  ∀ (Validator BlockId Payload : Type) [Fintype Validator] [DecidableEq Validator]
    ...
    Agreement U w ∧ Integrity U w ∧ Validity U w ∧ ValidityOfEventualReference U w ∧
      TotalOrder U w
