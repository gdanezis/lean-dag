...
theorem spansEligible_fourOpt : SpansEligibleAt (Validator := Fin 4) 2 3 := by
  intro b i h
  change i + 2 < b + 3 - 1
  omega

/-- ... and at `c = 4`. -/
theorem spansEligible_fourOpt4 : SpansEligibleAt (Validator := Fin 4) 2 4 := by
  intro b i h
  change i + 2 < b + 4 - 1
  omega
...
example : ∀ i, i < 1 → ∃ v, DecidedOpt OE VE i v :=
  (OptimalHydrozoan.IndirectLiveness.holds (Fin 4) (Fin 22) OE).2 VE 1 3 (by omega)
    ...
    (fun j h1 h2 => by
      ...
      · exact ⟨4, DecidedOpt.directCommit (by decide) (Or.inl (by decide))⟩
      · exact ⟨10, DecidedOpt.directCommit (by decide) (Or.inl (by decide))⟩
      · exact ⟨14, DecidedOpt.directCommit (by decide) (Or.inl (by decide))⟩)
example : ∀ i, i < 1 → ∃ v, DecidedOpt OE VE i v :=
  (OptimalHydrozoan.IndirectLiveness.holds (Fin 4) (Fin 22) OE).2 VE 1 4 (by omega)
    ...
    (fun j h1 h2 => by
      ...
      · exact ⟨10, DecidedOpt.directCommit (by decide) (Or.inl (by decide))⟩
      · exact ⟨14, DecidedOpt.directCommit (by decide) (Or.inl (by decide))⟩
      · exact ⟨18, DecidedOpt.directCommit (by decide) (Or.inl (by decide))⟩)
