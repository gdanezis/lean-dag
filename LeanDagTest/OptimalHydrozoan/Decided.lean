...
equivocation) exercising every route of `DecidedOpt`, anchored on
slot 6 (id 22), fast-committed at exactly `qFastOpt` votes — one fewer
than Hydrozoan's `qFast`, pinned side by side:

* slot 0 (candidate 3): the direct commit by the fast path and by the slow one;
* slot 1 (candidate 29, which no block ever references): the direct skip,
  and the indirect skip anchored on slot 6 with both rungs **refuted for the
...
* slot 2 (candidate 8, FinWhale's Figure 4): a single correct vote (id 14)
  ...
  the evidence rung is the **only** route;
* slot 3 (candidate 13): the certificate rung through certificate 18, a
  parent of the anchor — at `n = 4`, where `qCert = qFastOpt`, both direct
  commits are derivable too (the relation is order-free);
* slot 6: the fast path; a view withholding one vote has `2 < 3`.
...
reference 4, 5, 6 — LeanDag.Hydrozoan.certificates for 3; id 8 is slot 2's candidate. Round 3: ids 11–13 by
...
18–20 by `1`, `2`, `3` reference 15, 16, 17 — LeanDag.Hydrozoan.certificates for 13. Round 6:
...
example : DecidedOpt OD VD 0 (some 3) :=
  DecidedOpt.directCommit (by decide) (Or.inr (by decide))
...
example :
    EligibleAt (Validator := Fin 4) 2 1 6 ∧ EligibleAt (Validator := Fin 4) 2 2 6 ∧
      EligibleAt (Validator := Fin 4) 2 3 6 ∧ ¬ EligibleAt (Validator := Fin 4) 2 3 5 ∧
...
-- The evidence rung end to end: slot 2 commits 8 anchored on slot 6; the
...
-- The certificate rung end to end; no slot between 3 and 6 is eligible.
example : DecidedOpt OD VD 3 (some 13) :=
  DecidedOpt.indirectCommit (j := 6) (A := 22) (i := 0) (by omega) (by decide)
    (DecidedOpt.directCommit (by decide) (Or.inl (by decide)))
    (fun i h1 h2 h3 => by
    ...
    (by decide)
    (show LeanDag.Hydrozoan.CertifiedIn UD 22 13 3 from ⟨18, by decide, Reaches.single (by decide)⟩)
    (fun _ _ _ h => h)
...
-- The indirect skip end to end: slot 1 anchored on slot 6 with the eligible
