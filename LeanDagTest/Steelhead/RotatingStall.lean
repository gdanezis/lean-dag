...
* **every period Algorithm 2 derives is `4`**, at every horizon, in every view and for every
  coin: the window of an anchor spans at most eight rounds, on which periods `1` and `2` score
  at least half of what period `4` can (`anchorUpdate_half_retains`);
...
def rtConfig : Config (Fin 4) := ⟨3, 5, 1, rtKnown⟩
