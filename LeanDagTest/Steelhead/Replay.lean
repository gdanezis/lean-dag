...
`sh8`, the eight-round healthy universe of `Model.lean`, over the last eight rounds: rounds `1`
to `7`, round `0` excluded. With the waves `3` and `5`, the canary `7` and the known leader
`(r + 1) mod 4`:
...
def rpConfig : Config (Fin 4) := ⟨3, 5, 7, fun r => ⟨(r + 1) % 4, by omega⟩⟩

/-- **The window**: the causal history of block `28` of `sh8`, at round `7`, over eight rounds. -/
