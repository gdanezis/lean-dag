...
> to the same question were explored and set aside — composing
> `Adaptive.Policy` with Barnacle, and one policy emitting the whole
> schedule at a two-epoch lag. Both carry the assumption §1 is about;
> this one does not, which is why it is the one recorded. §9 compares
> them.
...
The Barnacle paper answers it for §21's mechanism, and the answer is this
design. Its `TryCommit` walks the decision sequence *in order up to the
first undecided slot*, and the pivot at which the leader count changes is
a leader it has committed; so at the moment of the switch every slot
below the pivot is decided. Its `TryDecide` stops at the committed
prefix and never re-derives below it. A frontier and immutability, which
is §2 and §8 of this document. `Barnacle.sched_local` is the Lean
statement of why that makes `PartialRun.closed` sound.
...
segment, which is derivable from the fairness clause: a stretch of `c`
slots fitting inside a segment is `c` slots being there to fit in. And `f` must give a non-empty
...
| | composed with Barnacle | one policy, two-epoch lag | Barbouni |
|:---|:---|:---|:---|
